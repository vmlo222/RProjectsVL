...
Now lets rearrange the Bankruptcy Dataset
...
Change the d variable to numeric and the e variable to a factor
```{r }
df.5$d<-as.numeric(df.5$d)
df.5$e<-as.factor(df.5$e)
```

Now, lets create a second 3D scatter plot showing bankruptcy and nonbankruptcy
...
Reset the plot parameters back to the default settings
```{r }
dev.off()
```
